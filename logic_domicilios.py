...
def generar_pdf_domicilios(df, fecha_tit):
    ...
    for banda in df_logistica['BANDA HORARIA'].unique():
        ...
        def dibujar_encabezado_banda(continua=False):
            pdf.set_fill_color(0, 70, 145)
            pdf.set_text_color(255, 255, 255)
            pdf.set_font('Arial', 'B', f_size_datos + 2)

            texto = f"--- Domicilio | {banda} ---"
            if continua:
                texto += " (continúa...)"

            pdf.cell(0, h_celda + 1, texto, 1, 1, 'C', True)
        ...
        for i, row in df_banda.iterrows():
            ...
            pdf.cell(w_num, h_celda, str(contador), 1, 0, 'C', fill)
            ...
            # Detectar salto automático de página
            if pdf.get_y() < y_antes:
                dibujar_encabezado_banda(continua=True)
                dibujar_encabezado_tabla()
