def generar_pdf_clientes(df, fecha_tit):
    # 1. Forzar limpieza de nulos para evitar errores de tipo string
    df = df.fillna('')

    # 2. Lógica de unificación (Etiqueta única para Drive y Sucursal)
    ...
    # ORDENAMIENTO: Crucial para que no se repitan zócalos grises
    df = df.sort_values(by=['TIPO_ORDEN', 'BANDA HORARIA'], ascending=[False, True])

    pdf = PlanillaPDF(fecha_tit)
    pdf.add_page()
    widths = [28, 20, 32, 22, 22, 47, 25]
    ultima_llave = None
    resumen_unificado = {} # Diccionario para el conteo final

    for _, row in df.iterrows():
        llave_actual = row['LLAVE_ZOCALO']
        
        # CONTEO: Usamos la llave unificada para que el resumen no separe Drive de Sucursal
        resumen_unificado[llave_actual] = resumen_unificado.get(llave_actual, 0) + 1

        # DIBUJAR ZÓCALO: Solo si cambia la llave unificada
        ...
        pdf.cell(widths[6], 5, str(row.get('TEL. PARTICULAR', '')).split('.')[0][:13], border=1)
        pdf.ln()

    # INFORME FINAL (Resumen unificado)
    ...
    # CORRECCIÓN PARA fpdf2: Quitamos el .encode() porque output() ya devuelve bytes
    return pdf.output()
