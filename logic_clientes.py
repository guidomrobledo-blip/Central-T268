def generar_pdf_clientes(df, fecha_tit):
    # 1. Asegurar que no haya valores nulos que rompan el texto
    df = df.fillna('')

    # 2. Lógica de unificación para Zócalo y Resumen
    ...
    # 3. ORDENAMIENTO CRUCIAL: Esto garantiza que Drive y Sucursal queden juntos
    df = df.sort_values(by=['PRIORIDAD', 'BANDA HORARIA'], ascending=[False, True])
    ...
    # EL ARREGLO PARA EL ERROR (errors='ignore')
